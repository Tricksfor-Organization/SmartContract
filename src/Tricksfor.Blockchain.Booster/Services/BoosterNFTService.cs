...
namespace Tricksfor.Blockchain.Booster.Services
{
    ...
    /// </summary>
    public class BoosterNFTService
    {
        private readonly IWeb3 _web3;
        private readonly string _contractAddress;

        public BoosterNFTService(IWeb3 web3, string contractAddress)
        {
            _web3 = web3;
        ...
        public Task<bool> PausedQueryAsync()
            => _web3.Eth.GetContractQueryHandler<NftPausedFunction>()
                .QueryAsync<bool>(_contractAddress, new NftPausedFunction());
        ...
        public Task<TransactionReceipt> PauseRequestAndWaitForReceiptAsync()
            => _web3.Eth.GetContractTransactionHandler<NftPauseFunction>()
                .SendRequestAndWaitForReceiptAsync(_contractAddress, new NftPauseFunction());

        public Task<TransactionReceipt> UnpauseRequestAndWaitForReceiptAsync()
            => _web3.Eth.GetContractTransactionHandler<NftUnpauseFunction>()
                .SendRequestAndWaitForReceiptAsync(_contractAddress, new NftUnpauseFunction());
    }
}
